...
```{r}
data_dir       <- "/Users/mdozmorov/Documents/Data/VCU_work/Tony/2023-10.RNA-seq" # Working directory
fileNameIn1 <- file.path(data_dir, "OUT_full/star_rsem/rsem.merged.gene_counts.tsv") # Counts
fileNameIn2 <- file.path(data_dir, "OUT_full/star_rsem/rsem.merged.gene_tpm.tsv") # TPM
...
if (human_analysis) {
  ...
  selected_annotations <- c("symbol", "biotype", "description")
...
if (mouse_analysis) {
  ...
  gene_annotations <- grcm38[ !(grepl("_", grcm38$chr) | grepl("GL", grcm38$chr)), c("symbol", "biotype", "description")]
...
sample_annotation$Sample <- basename(sample_annotation$fastq_1) %>% sub("_S[0-9]+_R1_001.fastq.gz", "", .) %>% make.names()
...
counts_average <- left_join(counts_average, gene_annotations[, c("symbol", "description")], by = c("Geneid" = "symbol"))
```
...
## Analysis 1, HSRNAtr vs HSRNAun

"HSRNAtr" vs. "HSRNAun" comparison, so positive logFC correspond to genes UP in "HSRNAtr"
...
res.full <- p.vals(object = lrt, p_adj_cutoff = 1, comparison = analysis_ID, print_to_file = FALSE, merge_by = "symbol")
...
```{r fig.height=12}
if (nrow(res) > 0)
  genes_to_heatmap(object = res, edgeR.dgelist = edgeR.dgelist, subset_by = c(group_trt, group_ref), nplot = nplot, comparison = analysis_ID, clustmethod = "ward", width = 6, height = 8, print_to_file = FALSE)
```
