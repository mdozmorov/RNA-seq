...
```{r}
data_dir       <- "/Users/mdozmorov/Documents/Data/VCU_work/Shawn_Wang/2025-07.RNA-seq" # Working directory
fileNameIn1 <- file.path(data_dir, "OUT_mm39/star_rsem/rsem.merged.gene_counts.tsv") # Counts
fileNameIn2 <- file.path(data_dir, "OUT_mm39/star_rsem/rsem.merged.gene_tpm.tsv") # TPM
...
if (mouse_analysis) {
  ...
  gene_annotations <- grcm38[ !(grepl("_", grcm38$chr) | grepl("GL", grcm38$chr)), selected_annotations]
...
sample_annotation$Sample <- basename(sample_annotation$fastq_1) %>% sub("_[12].fastq.gz", "", .) %>% make.names()
# Explicitly make factors. Manually adjust for all columns in the sample annotation file
sample_annotation$Group   <- factor(sample_annotation$Sample %>% sub("[0-9]", "", .))
```
...
if (saveTPM & !file.exists(fileName_tpm)) {
  tpm_data <- read_tsv(fileNameIn2)
  TPM <- data.frame(Geneid = tpm_data$gene_id, log2(tpm_data[, colnames(tpm_data)[!(colnames(tpm_data) %in% c("gene_id", "transcript_id(s)"))]] + 1))
  # Save the data
...
```{r eval=FALSE}
samples_to_exclude <- c("MYCi975.2")
...
counts_average <- left_join(counts_average, gene_annotations[, c("symbol", "ensgene", "description")], by = c("Geneid" = "ensgene"))
```
...
"KO" vs. "WT" comparison, so positive logFC correspond to genes UP in "KO"
...
res.full <- p.vals(object = lrt, p_adj_cutoff = 1, comparison = analysis_ID, print_to_file = FALSE, merge_by = "ensgene")
...
Total number of differentially expressed genes: `r nrow(res)`, `r nrow(res[ res$logFC > 0, , drop = FALSE])` are upregulated, `r nrow(res[ res$logFC < 0, , drop = FALSE])` are downregulated. Check the following for proportion changes.
...
if (nrow(res) > 0) {
  ...
  genes_to_boxplot(selected_genes = selected_genes[1:min(length(selected_genes), nbox)], subset_by = c(group_trt, group_ref), merge_by = "ensgene")
}
```
...
```{r fig.height=12}
if (nrow(res) > 0)
  genes_to_heatmap(object = res, edgeR.dgelist = edgeR.dgelist, subset_by = c(group_trt, group_ref), nplot = nplot, comparison = analysis_ID, clustmethod = "ward", width = 6, height = 8, print_to_file = FALSE, merge_by = "ensgene")
```
...
if( run_gsea == FALSE) {
  ...
  if(websiteLive) {
    ...
    up.genes <- sort(unique(degs_subset$genes[ degs_subset$logFC > 0 ]))
    dn.genes <- sort(unique(degs_subset$genes[ degs_subset$logFC < 0 ]))
    # Run KEGG
    if (up_dn_separate) {
