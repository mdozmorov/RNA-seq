...
```{r settings}
# Data
data_dir       <- "/Users/mdozmorov/Documents/Data/VCU_work/Tony/2023-06.ATAC-seq" # Working directory
p_adj_cutoff   <- 0.05 # FDR cutoff
fileNameIn1   <- file.path(data_dir, "results/Integration_SAN_DAC_vs_CTL_full.xlsx") # File to save all results of differential expression analysis
# degs_sheet     <- "SAN_DAC_results_all" # Which worksheet contains differentially expressed genes
fileNameOut1 <- file.path(data_dir, "results/GSEA_Integration_SAN_DAC_vs_CTL.xlsx")
...
FDR_cutoff     <- 0.1 # FDR cutoff for enrichment network
min_enrichments <- 20 # Minumum number of enrichments to plot
max_enrichments <- 100 # Maximum number of enrichments to plot
...
```{r}
mtx.pos <- read_xlsx(fileNameIn1, sheet = "POS.POS")
mtx.neg <- read_xlsx(fileNameIn1, sheet = "NEG.NEG")
mtx.neg$RanklogFC.Mean <- -1 * mtx.neg$RanklogFC.Mean # Negate the rank for negative associations
mtx.neg <- mtx.neg[order(mtx.neg$RanklogFC.Mean, decreasing = TRUE), ] # Reverse the order

mtx <- data.frame(rbind(mtx.pos, mtx.neg))
res <- data.frame(symbol = mtx$Gene.Symbol, logFC = mtx$RanklogFC.Mean, p.adj = mtx$MeanlogFC.RNA)
...
websiteLive <- TRUE # Check if EnrichR is up
  ...
  if(websiteLive) {
    ...
    if (nrow(res) > max_kegg_genes) {
      degs_subset <- rbind(head(res, n = max_kegg_genes / 2),
                           tail(res, n = max_kegg_genes / 2))
    } else {
      degs_subset <- res
    }
...
gsea_kegg_enrich <- function(dataset, p_adj_cutoff = p_adj_cutoff) {
  ...
  ego3 <- gseKEGG(geneList     = geneList,
                  ...
                  pvalueCutoff = 1,
                  verbose      = FALSE)
  # Save enrichment object for plotting
  enrichment <- ego3
  # Get summary
  ...
  return(list(res.kegg.gsea, enrichment))
}

# run GSEA KEGG analysis 
res.enrich <- gsea_kegg_enrich(res, p_adj_cutoff = p_adj_cutoff)
res.kegg.gsea <- res.enrich[[1]]
enrichment <- res.enrich[[2]]
```

### Enrichment network

```{r}
# How many enrichments are significant
sig_enrichments <- sum(enrichment@result$p.adjust < FDR_cutoff)

if (sig_enrichments > max_enrichments) {
  # If too many enrichments, process top max
  print(paste("Too many significant enrichments at FDR", FDR_cutoff, ",", sig_enrichments, "total"))
  print(paste("Plotting top", max_enrichments))
  p <- enrichmentNetwork(as.data.frame(enrichment@result[1:max_enrichments, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle("GSEA.KEGG")
  as.widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size'))) 
} else if (sig_enrichments < min_enrichments) {
  # If too few enrichments, process top max
  print(paste("Too few significant enrichments at FDR", FDR_cutoff, ",", sig_enrichments, "total"))
  print(paste("Plotting top", min_enrichments))
  p <- enrichmentNetwork(as.data.frame(enrichment@result[1:min_enrichments, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle("GSEA.KEGG")
  as.widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size')))
} else {
  # If the number of enrichments within min-max limit
  print(paste("The numer of significant enrichments at FDR", FDR_cutoff, "is", sig_enrichments))
  print(paste("Plotting all", sig_enrichments, "of them"))
  p <- enrichmentNetwork(as.data.frame(enrichment@result[enrichment@result$p.adjust < FDR_cutoff, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle("GSEA.KEGG")
  as.widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size')))
}
```
...
gsea_msigdb_enrich <- function(dataset, p_adj_cutoff = p_adj_cutoff) {
  ...
  for (gs_cat in m_df_gs_cat) {
    ...
    if (nrow(em2@result) > 0) {
      # Save enrichments
      enrichment <- c(enrichment, list(em2))
      # Reformat results
  ...
  return(list(res.msigdf.all, enrichment))
}

# run GSEA MSigDb
res.enrich <- gsea_msigdb_enrich(res, p_adj_cutoff = p_adj_cutoff)
res.msigdb.gsea <- res.enrich[[1]]
enrichment <- res.enrich[[2]]
```
...
for (i in 1:length(enrichment)) {
  name_enrichment <- names(res.msigdb.gsea[i])
  # How many enrichments are significant
  sig_enrichments <- sum(enrichment[[i]]@result$p.adjust < FDR_cutoff)
  
  if (sig_enrichments > max_enrichments) {
    # If too many enrichments, process top max
    print(paste(name_enrichment, ": Too many significant enrichments at FDR", FDR_cutoff, ",", sig_enrichments, "total"))
    print(paste(name_enrichment, ": Plotting top", max_enrichments))
    p <- enrichmentNetwork(as.data.frame(enrichment[[i]]@result[1:max_enrichments, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle(name_enrichment)
    plt[[i]] <- as_widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size')))
  } else if (sig_enrichments < min_enrichments) {
    # If too few enrichments, process top max
    print(paste(name_enrichment, ": Too few significant enrichments at FDR", FDR_cutoff, ",", sig_enrichments, "total"))
    print(paste(name_enrichment, ": Plotting top", min_enrichments))
    p <- enrichmentNetwork(as.data.frame(enrichment[[i]]@result[1:min_enrichments, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle(name_enrichment)
    plt[[i]] <- as_widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size')))
  } else {
    # If the number of enrichments within min-max limit
    print(paste(name_enrichment, ": The number of significant enrichments at FDR", FDR_cutoff, "is", sig_enrichments))
    print(paste(name_enrichment, ": Plotting all", sig_enrichments, "of them"))
    p <- enrichmentNetwork(as.data.frame(enrichment[[i]]@result[enrichment[[i]]@result$p.adjust < FDR_cutoff, ]), drawEllipses = TRUE, fontSize = 2.5) + ggtitle(name_enrichment)
    plt[[i]] <- as_widget(ggplotly(p, tooltip=c('ID', 'Cluster', 'Cluster size')))
  }
}
...
for (gs_selected in gs_selected_all) {
  ...
  m_t2g <- msigdbr(species = "Homo sapiens", category = gs_cat) %>% dplyr::distinct(gs_name, entrez_gene) %>% dplyr::filter(gs_name == !!gs_selected)
  em2 <- GSEA(geneList, TERM2GENE = m_t2g, pvalueCutoff = p_adj_cutoff)
  # Save the picture
