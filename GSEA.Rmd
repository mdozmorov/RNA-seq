...
fileNameIn1   <- file.path(data_dir, paste0("results/DEGs.xlsx")) # File to save all results of differential expression analysis
degs_sheet     <- "1.DEGs" # Which worksheet contains differentially expressed genes
fileNameOut1 <- file.path(data_dir, paste0("results/GSEA_DEGs_", degs_sheet, ".xlsx"))
...
res <- data.frame(symbol = mtx$genes, logFC = mtx$logFC, p.val = mtx$PValue, p.adj = mtx$FDR)
...
websiteLive <- TRUE # Check if EnrichR is up
  ...
  if(websiteLive) {
    ...
    if (nrow(res[res$p.adj < FDR_cutoff, ]) > max_kegg_genes) {
