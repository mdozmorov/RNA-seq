...
p.vals <- function(object, p_adj_cutoff = 0.1, subset_by = subset_by, comparison = "DEGs", nplot = 50, print_to_file = TRUE) {
...
genes_to_boxplot <- function(selected_genes = c("NF1", "SND1", "MTDH"), subset_by = c("C", "AA")) {
  ...
  matrix.to.plot_melted <- melt(matrix.to.plot, id.vars = "Gene", measure.vars = colnames(matrix.to.plot)[colnames(matrix.to.plot) != "Gene"])
  matrix.to.plot_melted$Gene     <- as.character(matrix.to.plot_melted$Gene) # factor(matrix.to.plot_melted$Gene, levels = unique(matrix.to.plot_melted$Gene))
  matrix.to.plot_melted$variable <- as.character(matrix.to.plot_melted$variable) # factor(matrix.to.plot_melted$variable %>% as.character(), levels = unique(matrix.to.plot_melted$variable %>% as.character()))
  # Plot
  ggplot(matrix.to.plot_melted, aes(x = variable, y = value, fill = variable)) +
    geom_boxplot() +
