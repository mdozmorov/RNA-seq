...
csl: /Users/mdozmorov/Documents/Work/presentations/Templates/manuscript_template/styles.ref/genomebiology.csl
output:
  pdf_document:
    toc: no
#  word_document:
...
```{r}
data_dir       <- "/Users/mdozmorov/Documents/Data/VCU_work/Sarkar/Devanand_2020-04/" # Working directory
...
saveTPM  <- FALSE # Whether or not to save expression counts to TPM
...
sample_annotation$Sample <- sub(".ReadsPerGene.out.tab", "", sample_annotation$Sample) # For STAR files
...
files_counts <- list.files(path = paste0(data_dir, "02_STAR-align/"), pattern = "*.tab.gz")
for (f in files_counts) {
  count_data <- read.table(paste0(data_dir, "02_STAR-align/", f), sep = "\t", header = FALSE)
  ...
  count_colnames <- c(count_colnames, sub(".ReadsPerGene.out.tab", "", f))
}
...
"T" vs. "S comparison, so positive logFC correspond to genes UP in "T"

```{r}
# Subset the data, adjust manually
index <- sample_annotation$Group == "T" | sample_annotation$Group == "S"
# Or, use all the data
...
res.full <- p.vals(object = lrt, p_adj_cutoff = p_adj_cutoff, comparison = "Analysis_1", print_to_file = FALSE)
...
Total number of differentially expressed genes: `r nrow(res.full)`, `r nrow(res.full[ res.full$logFC > 0, , drop = FALSE])` are upregulated, `r nrow(res.full[ res.full$logFC < 0, , drop = FALSE])` are downregulated.
```{r}
res <- res.full[ res.full$biotype == "protein_coding" & !is.na(res.full$symbol), ]
```
...
if (nrow(res) > 0) {
  ...
  genes_to_boxplot(selected_genes = selected_genes[1:min(length(selected_genes), nbox)], subset_by = c("T", "S"))
}
```
...
x1 <- list(res.full[res.full$biotype == "protein_coding" & !is.na(res.full$biotype), ],
           res.full[res.full$biotype == "lincRNA" & !is.na(res.full$biotype), ],
           res.full[!(res.full$biotype %in% c("protein_coding", "lincRNA")) & !is.na(res.full$biotype), ])
names(x1) <- c("1.protein-coding", "1.lincRNA", "1.other")
...
# Adjust as needed
pv.out <- pathview(gene.data = degs.genes, pathway.id = "04658", species = species, gene.idtype = "ENTREZ", gene.annotpkg = OrgDb, out.suffix = "Analysis_1")
```
