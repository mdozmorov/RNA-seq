...
genes_to_heatmap <- function(object, edgeR.dgelist = edgeR.dgelist, nplot = 50, subset_by = c("A", "B"), comparison = "C231_noCRISPR_vs_Negative", clustmethod = "ward", width = 6, height = 8, print_to_file = TRUE) {
  ...
  if (print_to_file) {
    ...
    pheatmap(matrix.to.plot, color=colorRampPalette(c('blue', 'white', 'red'))(20), clustering_method = "ward", treeheight_row = FALSE, treeheight_col = FALSE, annotation_col = annotation_col, labels_row = genes.to.plot$symbol, scale = "row")
  }
}
```
...
if( run_gsea == FALSE) {
  ...
  if (up_dn_separate) {
    ...
    res.kegg <- save_enrichr(up.genes = up.genes, dn.genes = dn.genes, databases = "KEGG_2016", fdr.cutoff = p_adj_cutoff, fileName = fileName_rna, wb = wb)
  } else {
...
if (run_gsea == TRUE) {
  ...
  # All DEGs
  res.all <- topTable(fit2, coef = 1, number = Inf)
  # Convert symbols to entrezids
  eid <- bitr(rownames(res.all), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
  # Attach converted entrezids
  res.all <- left_join(data.frame(SYMBOL = rownames(res.all), res.all), eid, by = "SYMBOL")
  res.all <- res.all[ res.all$ENTREZID != "", ]
