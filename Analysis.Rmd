...
```{r}
data_dir       <- "/Users/mdozmorov/Documents/Data/VCU_work/Devanand_2018_3/" # Working directory
...
index <- sample_annotation$Group == "T7" | sample_annotation$Group == "C2"
# Or, use all the data
...
if (nrow(res) > 0) {
  ...
  genes_to_boxplot(selected_genes = selected_genes[1:nbox], subset_by = c("C2", "T7"))
}
```
...
selected_genes <- c("TAF2", "MTDH", "SND1")
# Gen Ensembl IDs
...
colnames(selected_genes_expr) <- ifelse(sample_annotation_subset$Group == "C2", "C2", "T7") %>% as.character()
