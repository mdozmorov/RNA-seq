...
genes_to_boxplot <- function(selected_genes = c("NF1", "SND1", "MTDH"), subset_by = c("C", "AA")) {
  ...
  subset_index <- sample_annotation$Group == subset_by[1] | sample_annotation$Group == subset_by[2]
  ...
  colnames(matrix.to.plot) <- sample_annotation$Group[subset_index]
  # Add genes
...
Differentially espressed genes (DEGs) detected at False Discovery Rate (FDR) `r p_adj_cutoff`. Top `r top_n` or less DEGs are visualized.
...
```{r fig.height=12}
if (nrow(res) > 0) {
  res[, c(3, 4, 5)] <- round(res[, c(3, 4, 5)], 2)
  res[, c(6)] <- formatC(res[, c(6)], format = "e", digits = 2)
  res[, c(7)] <- formatC(res[, c(7)], format = "e", digits = 2)
...
```{r}
if (nrow(res) > 0)
  genes_to_boxplot(selected_genes = res$symbol[1:nbox], subset_by = c("C", "AA"))
```
...
### KEGG pathways for all differentially expressed genes

```{r}
if (nrow(res) > min_kegg_genes) {
    up.genes <- unique(res$symbol)[1:min(length(unique(res$symbol)), max_kegg_genes)]
    res.kegg <- save_enrichr(up.genes = up.genes, databases = "KEGG_2016", fileName = fileName_rna, wb = wb, sheetName = "KEGG_1" )
  if (nrow(res.kegg) > 0) {
    kable(res.kegg[1:min(nkegg, nrow(res.kegg)), ])
  }
} else {
  print("Insufficient data to run KEGG enrichment analysis")
}
```
...
selected_genes <- c("ISG15", "OAS1", "MX1", "IFI6", "IFIT1", "IFITM1", "IFIT3")
# Gen Ensembl IDs
...
colnames(selected_genes_expr) <- ifelse(sample_annotation$Group == "C", "White", "Black, or AA") %>% as.character()
