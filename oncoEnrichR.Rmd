...
```{r data}
# Input settings
dir_data <- "/Users/mdozmorov/Documents/Work/VCU_work/Tony/Sumoylation"
fileNameIn <- file.path(dir_data, "data/SKNSHMBvsSKNSHMA (1).xlsx")
# oncoEnrichR settings, depending on input gene IDs
query_id_type = "ensembl_gene"
# Output settings
analysis_name <- "SKNSHMBvsSKNSHMA"
dir_results   <- "/Users/mdozmorov/Documents/Work/GitHub/RNA-seq"
fileNameOutUP1  <- file.path(dir_results, "results", paste0("oncoEnrichR_", analysis_name, "_UP.html"))
fileNameOutUP2  <- file.path(dir_results, "results", paste0("oncoEnrichR_", analysis_name, "._UP.xlsx"))
fileNameOutDN1  <- file.path(dir_results, "results", paste0("oncoEnrichR_", analysis_name, "_DN.html"))
fileNameOutDN2  <- file.path(dir_results, "results", paste0("oncoEnrichR_", analysis_name, "._DN.xlsx"))
# Create "results" folder, if does not exist
dir.create(file.path(dir_results, "results"))

# Read the data
mtx <- read_xlsx(fileNameIn)
# Create a data frame for downstream processing - adjust column names
mtx <- data.frame(Gene = mtx$gene_id,
                  UPDN = ifelse(mtx$log2FoldChange > 0, "Upregulated", "Downregulated"),
                  FDR  = mtx$padj)
# Sort by FDR
...
oncoEnrichR_wrapper <- function(query = query_UP, 
...
report_UP <- oncoEnrichR_wrapper(query = query_UP, query_id_type = query_id_type, project_title = analysis_name)
oncoEnrichR::write(report = report_UP, file = fileNameOutUP1, format = "html")
oncoEnrichR::write(report = report_UP, file = fileNameOutUP2, format = "excel")
# Analysis of upregulated genes
report_DN <- oncoEnrichR_wrapper(query = query_DN, query_id_type = query_id_type, project_title = analysis_name)
oncoEnrichR::write(report = report_DN, file = fileNameOutDN1, format = "html")
oncoEnrichR::write(report = report_DN, file = fileNameOutDN2, format = "excel")
