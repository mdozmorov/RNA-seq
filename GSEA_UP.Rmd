...
entrez_to_symbols <- function(entrez_list, gene_annotations, ntop = 24) {
  # Create a lookup from entrezid to symbol using gene_annotations
  # Filter out NA entrezids
  entrez_map <- gene_annotations %>%
  ...
  convert_single <- function(entrez_string) {
    ...
    # Match against gene_annotations
    symbols <- sapply(entrez_ids, function(eid) {
      match_symbol <- entrez_map$symbol[entrez_map$entrezid == eid]
      if (length(match_symbol) > 0) {
        return(match_symbol[1])
      } else {
        return(eid)  # fallback to ID if not found
      }
    })
