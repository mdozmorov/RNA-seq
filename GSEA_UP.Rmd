...
fileNameIn1   <- file.path(data_dir, "data/Proteomics_DE_analysis.xlsx") 
degs_sheet <- "DFMO_NORX"
degs_sheet <- "GC7_NORX"
degs_sheet <- "DFMO_GC7"
...
p_val_cutoff   <- 0.1 # FDR cutoff for DEGs
...
max_kegg_genes <- 500 # Maximum number of genes to run enrichment analysis on
...
fileNameOut1 <- file.path(data_dir, "GSEA_UP.pdf")
fileNameOut2 <- file.path(data_dir, paste0("GSEA2_", make.names(degs_sheet), ".pdf"))
...
res <- data.frame(symbol = mtx$Protein, logFC = mtx[, grepl("log2", colnames(mtx)), drop = TRUE], p.val = mtx$p.value, p.adj = mtx$FDR)
...
websiteLive <- TRUE # Check if EnrichR is up
  ...
  if(websiteLive) {
    ...
    if (nrow(res[res$p.val < p_val_cutoff, ]) > max_kegg_genes) {
      ...
      degs_subset <- res[res$p.adj < p_val_cutoff, ]
    }
