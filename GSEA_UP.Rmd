...
build_gene_data <- function(ensdb, orgdb, canonical_chrs, species_code, kegg_set, msigdbr_org) {
  ...
  # Map description (GENENAME) from org.*.eg.db using ENSEMBL keys
  desc_map <- AnnotationDbi::select(
    ...
    dplyr::rename(ensgene = ENSEMBL, description = GENENAME)
  ...
  gene_annotations <- df %>%
    # dplyr::filter(
      ...
      # description != ""
    # ) %>%
    distinct(ensgene, symbol, biotype, description, .keep_all = FALSE)
...
# Result objects:
# - gene_annotations: tibble with columns ensgene, symbol, biotype, description
...
gsea_msigdb_results_to_df <- function(res.msigdb.all, OrgDb, msigdbr_org) {
  res.msigdf.all <- list()
...
res.msigdb <- gsea_msigdb_results_to_df(res.msigdb.all = res.msigdb.em2, OrgDb = OrgDb, msigdbr_org = msigdbr_org)
